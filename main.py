...
def create_note():
    note_id = str(random.randint(0, 1000))
...
def save_note(note):
    with open("notes2.json", "a") as file:
        json.dump(note, file)
        file.write('\n')
        # file.write('*-*-*-*-*-*-*-*')
...
def read_notes():
    with open("notes2.json", "r") as file:
...
def save_notes_file(notes):
    with open("notes2.json", "w") as file:
        for note in notes:
            json.dump(note, file)
            file.write('\n')
